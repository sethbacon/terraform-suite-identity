...
| `identity/appcreds` | Mints access tokens for app-owned (headless) integrations: the Microsoft Entra client-credentials grant, the Entra workload-identity federated exchange (no stored secret at all), and GitHub App installation tokens. Mechanism only — the storage schema, validation rules and API-calling layer stay in the app (#206's split). A `Minter` holds no package state and caches nothing, because the two consumers cache very differently on purpose; `Cache` is an optional in-memory one keyed by credential fingerprint, so rotating any credential field is self-invalidating. Its default HTTP client is `httpsafe`-guarded: a token endpoint is exactly the operator-configurable URL that must not be pointed at an internal address. |
