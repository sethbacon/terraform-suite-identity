...
| `identity/store`     | The data-access layer (repository pattern) for those types, plus `TokenRepository` (JWT revocation). Repos use **unqualified** table names so the connection's `search_path` selects the schema — assert it with `identity.VerifySchemaRouting`. Also `Reducer`, the **transactional authority reduction**: because this module owns both `organization_members` and `api_keys`, removing a membership (or narrowing a role) and deleting the API keys that froze a snapshot of that authority are ONE transaction, so a process that dies mid-way cannot leave a credential holding authority the membership no longer grants. The app's own credential families join that transaction through `AppCredentials`, which is handed the live `*sql.Tx`; `NoAppCredentials` is the named opt-out, and `nil` is refused. The role-template family is the one reduction that cannot be a transaction — narrowing or deleting a template moves authority for every membership holding it — so its sanctioned path is `TemplateWriter`: it runs the bounded, resumable `ReconcileRoleTemplate` sweep first, refuses to write at all if that sweep did not finish, and only then issues the template statement. `PreviewRoleTemplateReconciliation` reports what a proposed edit would sweep, before it runs. |
...
| `identity/appcreds` | Mints access tokens for app-owned (headless) integrations: the Microsoft Entra client-credentials grant, the Entra workload-identity federated exchange (no stored secret at all), a certificate credential, a user-assigned managed identity, and GitHub App installation tokens. Mechanism only — the storage schema, validation rules and API-calling layer stay in the app (#206's split). A `Minter` holds no package state and caches nothing, because the two consumers cache very differently on purpose; `Cache` is an optional in-memory one keyed by credential fingerprint, so rotating any credential field is self-invalidating. Its default HTTP client is `httpsafe`-guarded: a token endpoint is exactly the operator-configurable URL that must not be pointed at an internal address. |
